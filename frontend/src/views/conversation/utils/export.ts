...
export const saveAsMarkdown = (convHistory: BaseConversationHistory, addTitle = true) => {
  ...
  content += `Date: ${create_time}\nModel: ${getChatModelNameTrans(convHistory.current_model || null, convHistory.source || null)}\n`;
    ...
    for (const group of groups) {
      ...
      if (displayType === 'text') {
        ...
        for (const part of imageParts) {
          imageContent += `\`[Image: ${part.width}*${part.height}${part.metadata ? ', ' + JSON.stringify(part.metadata) : ''
            }]\`\n\n`;
        }
